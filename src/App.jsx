...
import {
  ...
  CheckCircle2, ChevronLeft, ChevronRight, Menu, X,
} from 'lucide-react';
...
const Overview = () => {
  ...
  const kpis = [
    ...
    { icon: Clock, label: "Today's Punches", value: todayRecs.length, sub: `${onTimeToday} on-time`, color: '#D4AF37' },
    { icon: Calendar, label: 'Monthly Records', value: monthRecs.length, sub: monthStr, color: '#06b6d4' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }}
        style={{
          padding: '1px', borderRadius: 24,
          background: 'linear-gradient(135deg, rgba(212,175,55,0.5), rgba(99,102,241,0.35), rgba(16,185,129,0.2))',
          boxShadow: '0 20px 60px rgba(0,0,0,0.5)'
        }}>
        <div className="hero-inner">
          <div style={{ position: 'absolute', top: -40, left: -40, width: 260, height: 260, background: 'rgba(212,175,55,0.12)', filter: 'blur(70px)', borderRadius: '50%', pointerEvents: 'none' }} />
          <div style={{ position: 'relative', zIndex: 1 }}>
            <h1 className="hero-title">
              Welcome back, <span style={{ background: 'linear-gradient(135deg,#D4AF37,#F5D070)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', filter: 'drop-shadow(0 0 16px rgba(212,175,55,0.5))' }}>
          ...
          <div className="hero-date-block">
            ...
            <p style={{ margin: 0, fontSize: '1.3rem', fontWeight: 800, color: '#f8fafc' }}>{format(now, 'MMMM do, yyyy')}</p>
          </div>
        </div>
      </motion.div>
        ...
        {kpis.map(({ icon: Icon, label, value, sub, color }, i) => (
          <Tilt key={label} tiltMaxAngleX={5} tiltMaxAngleY={5} scale={1.02} transitionSpeed={2000}>
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
              ...
              <p style={{ margin: 0, fontSize: '0.72rem', color, fontWeight: 600 }}>{sub}</p>
            </motion.div>
          </Tilt>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem' }}>
        ...
        {topPerformer && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
            ...
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, rgba(212,175,55,0.05) 0%, rgba(99,102,241,0.03) 100%)', pointerEvents: 'none' }} />
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <div style={{ position: 'relative' }}>
                <Ring pct={topPerformer.pct} color="#D4AF37" size={72} stroke={6} />
                <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.7rem', fontWeight: 900, color: '#D4AF37' }}>
                  {Math.round(topPerformer.pct)}%
                </div>
              </div>
              <div>
                <p style={{ margin: '0 0 0.2rem', fontWeight: 900, fontSize: '1.1rem', color: '#f8fafc' }}>{topPerformer.name}</p>
                <p style={{ margin: '0 0 0.25rem', fontSize: '0.75rem', color: '#64748b' }}>{fmtHrs(topPerformer.hrs)} worked · {topPerformer.days} days</p>
                <p style={{ margin: 0, fontSize: '0.78rem', color: '#D4AF37', fontWeight: 700 }}>
                  {Math.round(topPerformer.pct)}% of monthly target
                </p>
              </div>
            </div>
          </motion.div>
          ...
          {employees.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
              {[
                ...
                {
                  label: 'Avg Hours/Day', value: monthRecs.length > 0
                    ? fmtHrs(monthRecs.reduce((s, r) => s + calcHours(r.inTime, r.outTime), 0) / (new Set(monthRecs.map(r => r.date)).size || 1))
                    : '—', color: '#D4AF37'
                },
              ].map(s => (
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
        ...
        {todayRecs.length === 0 ? (
          <div className="insight-card" style={{ margin: '1rem', padding: '2rem' }}>
            <div className="insight-glow" style={{ background: 'radial-gradient(circle, rgba(212,175,55,0.08) 0%, transparent 70%)' }} />
            ...
            <p style={{ margin: 0, color: '#475569', fontSize: '0.8rem' }}>Attendance records will appear here in real-time.</p>
          </div>
        ) : (
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }}
        ...
        {payroll.length === 0 ? (
          <div className="insight-card" style={{ margin: '1rem' }}>
            ...
            <div className="insight-icon-ring"><BarChart2 size={28} color="#D4AF37" /></div>
            <h3 style={{ margin: '0 0 0.4rem', fontSize: '1rem', fontWeight: 800 }}>No Attendance Data Yet</h3>
            <p style={{ margin: 0, color: '#475569', fontSize: '0.82rem' }}>Add employees and log attendance to see the leaderboard.</p>
          </div>
        ) : (
          <div style={{ padding: '0.75rem' }}>
            {payroll.map((emp, i) => (
              <motion.div key={emp.id} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 + i * 0.05 }}
                style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '0.75rem', borderRadius: 14, marginBottom: i < payroll.length - 1 ? '0.4rem' : 0, transition: 'background 0.2s' }}
                onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.03)'}
                onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                <div style={{
                  width: 28, height: 28, borderRadius: '50%',
                  background: i === 0 ? 'rgba(212,175,55,0.15)' : i === 1 ? 'rgba(139,92,246,0.12)' : 'rgba(255,255,255,0.04)',
                  border: `1px solid ${i === 0 ? 'rgba(212,175,55,0.4)' : i === 1 ? 'rgba(139,92,246,0.25)' : 'rgba(255,255,255,0.06)'}`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: '0.7rem', fontWeight: 900,
                  color: i === 0 ? '#D4AF37' : i === 1 ? '#a78bfa' : '#475569', flexShrink: 0
                }}>
                  {i + 1}
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexShrink: 0 }}>
                  <div style={{ position: 'relative', width: 36, height: 36 }}>
                    <Ring pct={emp.pct} color={i === 0 ? '#D4AF37' : '#8b5cf6'} size={36} stroke={3} />
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.48rem', fontWeight: 900, color: i === 0 ? '#D4AF37' : '#a78bfa' }}>
                      {Math.round(emp.pct)}%
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <p style={{ margin: 0, fontWeight: 900, fontSize: '0.95rem', color: i === 0 ? '#D4AF37' : '#a78bfa' }}>{fmtHrs(emp.hrs)}</p>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem' }}>
        <div>
          <h2 style={{ margin: '0 0 0.15rem', fontWeight: 900, fontSize: '1.4rem' }}>
            Employee <span style={{ background: 'linear-gradient(135deg,#D4AF37,#F5D070)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Management</span>
          </h2>
        ...
        <div style={{ display: 'flex', gap: '0.6rem', flexWrap: 'wrap' }}>
          <div style={{ position: 'relative' }}>
            ...
            <input className="ipt" placeholder="Search…" value={search} onChange={e => setSearch(e.target.value)} style={{ paddingLeft: '2rem', width: 180 }} />
          </div>
      ...
      <AnimatePresence>
        {addOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ overflow: 'hidden' }}>
            <div style={{ padding: '1px', borderRadius: 18, background: 'linear-gradient(135deg, rgba(212,175,55,0.45), rgba(99,102,241,0.3))' }}>
              <div className="glass" style={{ borderRadius: 17, padding: '1.25rem 1.5rem' }}>
                <h3 style={{ margin: '0 0 1rem', fontWeight: 800, fontSize: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <Plus size={16} color="#D4AF37" /> New Employee
                </h3>
          ...
          return (
            <Tilt key={emp.id} tiltMaxAngleX={4} tiltMaxAngleY={4} scale={1.01} transitionSpeed={2500}>
              <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.04 * i }}
                className="glass" style={{ borderRadius: 18, padding: '1.25rem', position: 'relative', overflow: 'hidden', height: '100%' }}>
                <div style={{ position: 'absolute', top: -25, right: -25, width: 100, height: 100, background: 'rgba(212,175,55,0.08)', borderRadius: '50%', filter: 'blur(28px)', pointerEvents: 'none' }} />
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem', marginBottom: '1rem' }}>
                  <div style={{ position: 'relative', flexShrink: 0 }}>
                    <Ring pct={pct} color="#D4AF37" size={52} stroke={4} />
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%', margin: 5, background: 'linear-gradient(135deg,#B8962E,#6d28d9)', fontWeight: 900, fontSize: '1.1rem' }}>
                  ...
                  <button className="btn btn-red" onClick={() => confirm(`Remove ${emp.name}?`) && removeEmployee(emp.id)} style={{ padding: '0.3rem 0.5rem', flexShrink: 0 }}>
                    <Trash2 size={13} />
                  </button>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.4rem' }}>
                  {[
                    ...
                    { label: 'Hours', value: fmtHrs(hrs), color: '#D4AF37' },
      ...
      <div className="glass" style={{ borderRadius: 18, overflow: 'hidden', marginBottom: '1rem' }}>
        ...
        <div style={{ padding: '1.25rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.1rem' }}>
          <Field label="Office Opens At" hint="Late detection is based on this time ± grace period">
            <input type="time" className="ipt in-time" value={st.officeStartTime} onChange={e => setSt({ ...st, officeStartTime: e.target.value })} style={{ fontFamily: 'monospace', fontWeight: 700 }} />
          </Field>
...
const AttendanceTab = () => {
  ...
  useEffect(() => {
    ...
    employees.forEach(emp => {
      ...
      initial[emp.id] = rec ? { inTime: rec.inTime || '', outTime: rec.outTime || '' } : { inTime: '', outTime: '' };
    });
    setDrafts(initial);
    setSaved({});
  ...
  const saveAll = () => employees.forEach(emp => { const d = drafts[emp.id] || {}; if (d.inTime && d.outTime) saveRow(emp); });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <h2 style={{ margin: '0 0 0.15rem', fontWeight: 900, fontSize: '1.4rem' }}>
            Daily <span style={{ background: 'linear-gradient(135deg,#D4AF37,#F5D070)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Attendance</span>
          </h2>
          <p style={{ margin: 0, color: '#64748b', fontSize: '0.82rem' }}>Tap a card · Set In & Out time · Save</p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <button className="btn btn-ghost" onClick={prevDay} style={{ padding: '0.45rem' }}><ChevronLeft size={16} /></button>
          <input type="date" className="ipt" value={date} onChange={e => setDate(e.target.value)} style={{ width: 148, textAlign: 'center', fontWeight: 700 }} />
          <button className="btn btn-ghost" onClick={nextDay} style={{ padding: '0.45rem' }}><ChevronRight size={16} /></button>
          {date === today && <span className="badge" style={{ background: 'rgba(212,175,55,0.12)', color: '#D4AF37', border: '1px solid rgba(212,175,55,0.3)' }}>Today</span>}
        </div>
      </div>

      {/* Date summary bar */}
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
        style={{ padding: '1px', borderRadius: 18, background: 'linear-gradient(135deg, rgba(212,175,55,0.4), rgba(99,102,241,0.25))' }}>
        <div className="glass" style={{ borderRadius: 17, padding: '1rem 1.5rem', display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <p style={{ margin: '0 0 0.1rem', fontWeight: 900, fontSize: '1.05rem', color: '#f8fafc' }}>{format(dateObj, 'EEEE, MMMM do')}</p>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                <div style={{ width: 100, height: 6, background: 'rgba(255,255,255,0.07)', borderRadius: 99, overflow: 'hidden' }}>
                  <motion.div initial={{ width: 0 }} animate={{ width: `${fillPct}%` }} transition={{ duration: 0.8 }}
                    style={{ height: '100%', borderRadius: 99, background: 'linear-gradient(90deg,#D4AF37,#F5D070)', boxShadow: '0 0 8px rgba(212,175,55,0.5)' }} />
                </div>
                <span style={{ fontSize: '0.75rem', fontWeight: 800, color: '#D4AF37', whiteSpace: 'nowrap' }}>{Math.round(fillPct)}%</span>
              </div>
      ...
      {employees.length === 0 ? (
        <div className="insight-card">
          <div className="insight-glow" style={{ background: 'radial-gradient(circle, rgba(212,175,55,0.1) 0%, transparent 70%)' }} />
          <div className="insight-icon-ring" style={{ borderColor: 'rgba(212,175,55,0.2)' }}>
            <ClipboardList size={30} color="#D4AF37" />
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '0.85rem' }}>
          {employees.map((emp, i) => {
            ...
            const hrs = calcHrs(inT, outT);
            const st = getStatusLocal(inT, outT);
            const late = isLate(inT);
            const isSv = saved[emp.id];
            const pct = Math.min((hrs / (settings?.hoursPerDay || 9)) * 100, 100);
            return (
              <motion.div key={emp.id} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                style={{ borderRadius: 18, background: isSv ? 'rgba(52,211,153,0.06)' : st.bg, border: `1px solid ${isSv ? 'rgba(52,211,153,0.3)' : st.color + '30'}`, padding: '1.1rem', position: 'relative', overflow: 'hidden', transition: 'background 0.35s, border-color 0.35s', boxShadow: st.key !== 'empty' ? `0 4px 20px -8px ${st.glow}` : 'none' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.9rem' }}>
                  <div style={{ position: 'relative', flexShrink: 0 }}>
                    ...
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: '0.9rem', background: 'linear-gradient(135deg,#6d28d9,#0e7490)', borderRadius: '50%', margin: 4 }}>
                      {emp.name.charAt(0).toUpperCase()}
                    </div>
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginBottom: '0.7rem' }}>
                  {[
                    ...
                    <div key={field}>
                      ...
                      <input type="time" value={val} onChange={e => setDraft(emp.id, field, e.target.value)} className={`ipt ${cls}`} style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: '0.9rem', padding: '0.5rem 0.6rem' }} />
                    </div>
                  ))}
                </div>
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                  <button className="btn btn-v" onClick={() => saveRow(emp)} disabled={!inT || !outT} style={{ flex: 1, padding: '0.5rem', fontSize: '0.8rem', fontWeight: 800 }}>
                    <CheckCircle2 size={13} /> {isSv ? '✓ Saved' : 'Save'}
                  </button>
                  {(inT || outT) && (
                    <button className="btn btn-red" onClick={() => clearRow(emp.id)} style={{ padding: '0.5rem 0.75rem', fontSize: '0.8rem' }}>
                      <Trash2 size={13} />
                    </button>
                  )}
                </div>
...
const App = () => {
  ...
  useEffect(() => {
    if (!user) {
      login({ id: 'mock-admin', name: 'adminAdmin', email: 'admin@Admin.com', role: 'admin' });
    }
  }, [user, login]);
